...
def build_bert_preprocessing_dag(
    dag_id,
    ngram_dataset,
    ...
    tables = ",".join(
        [
            f"{input_dataset_project_id}.{input_dataset_name}.{table}"
            for table in input_table.split(",")
        ]
    )

    dataflow_operator = DataflowStartFlexTemplateOperator(
        task_id="load_bert_preprocessing",
        body={
            "launchParameter": {
                "containerSpecGcsPath": dataflow_template_path,
                "jobName": ngram_dataset.replace("_", "-")
                + '-bert-preprocessing-{{ execution_date.strftime("%Y%m%d-%H%M%S") }}',
                "parameters": {
                    "input-table": tables,
